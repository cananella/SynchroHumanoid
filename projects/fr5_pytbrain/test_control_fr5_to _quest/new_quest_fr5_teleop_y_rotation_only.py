#!/usr/bin/env python3
"""
Quest-FR5 Teleoperation - Y-AXIS ROTATION ONLY (Safe Test Version)

특징:
- Position: Full 3-DOF (X, Y, Z)
- Rotation: Y-axis only (pitch 회전)
- 매우 낮은 스케일로 안전하게 테스트
"""
...
SCALE_ROTATION_Y = 0.5    # Y축 회전 스케일 (1/2)
...
# 데드존
MIN_DELTA_POS = 0.1       # 0.1mm 미만 무시
MIN_DELTA_ROT = 0.1       # 0.1도 미만 무시
...
class QuestFR5TeleopAsync:
    ...
    def transform_quest_to_fr5(self, quest_pos_delta, quest_rot_delta):
        ...
        fr5_rot = np.array([
            0,                                    # RX 비활성화
            quest_rot_delta[1] * SCALE_ROTATION_Y,   # RY만 활성화 (부호 반전 제거!)
            0                                     # RZ 비활성화
        ])

        # Rotation 데드존 + 안전 제한
        rot_magnitude = np.abs(fr5_rot[1])  # Y축만 체크
        if rot_magnitude < MIN_DELTA_ROT:
            fr5_rot = np.zeros(3)
        elif rot_magnitude > MAX_DELTA_ROT:
            print(f"   [회전 제한] {rot_magnitude:.2f}° → {MAX_DELTA_ROT}°")
            fr5_rot[1] = np.sign(fr5_rot[1]) * MAX_DELTA_ROT
    ...
    def calibrate(self, calibration_time=5.0):
        ...
        print(f"   Quest Y 회전:  {quest_rot_median[1]:.1f}°")
        print(f"   Robot TCP:     [{robot_tcp[0]:.1f}, {robot_tcp[1]:.1f}, {robot_tcp[2]:.1f}]")
        print(f"   Robot RY:      {robot_tcp[4]:.1f}°")
    ...
    def run(self):
        """메인 루프"""
        print("\n" + "=" * 70)
        print("🎮 Quest-FR5 Teleoperation (Y-AXIS ROTATION ONLY)")
        print("=" * 70)
        print("\n📖 사용법:")
        ...
        print("   5. [q] - 종료")
        print("\n✅ Position: Full 3-DOF (X, Y, Z)")
        print("⚠️  Rotation: Y-axis only (pitch, 매우 낮은 스케일)")
        print("   컨트롤러를 앞뒤로 기울이면 로봇이 pitch 회전합니다.")
        print(f"   스케일: {SCALE_ROTATION_Y} (1/10), 최대: {MAX_DELTA_ROT}°")
        print("=" * 70 + "\n")
            ...
            while self.running:
                ...
                if cmd == 'q':
                    ...
                    if not self.quest_connected:
                        ...
                        if self.calibrate():
                            print("🎬 텔레오퍼레이션 시작! (Position + Y-Rotation)")
                        else:
                            print("❌ 동기화 실패")
