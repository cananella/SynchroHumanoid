#!/usr/bin/env python3
"""
Quest-FR5 Teleoperation - Z-AXIS ROTATION ONLY (Safe Test Version)

특징:
- Position: Full 3-DOF (X, Y, Z)
- Rotation: Z-axis only (엔드이펙터 끝단 회전만)
- 가장 안전한 테스트 버전
"""
...
SCALE_ROTATION = 1.0      # 회전 스케일 (1:1 매칭)

# 안전 제한
MAX_DELTA_POS = 100.0     # 최대 100mm
MAX_DELTA_ROT = 5.0       # 최대 5도 (더 크게!)

# 데드존
MIN_DELTA_POS = 0.1       # 0.1mm 미만 무시
MIN_DELTA_ROT = 0.1       # 0.1도 미만 무시 (더 크게)
...
class QuestFR5TeleopAsync:
    ...
    def transform_quest_to_fr5(self, quest_pos_delta, quest_rot_delta):
        ...
        fr5_rot = np.array([
            0,                       # RX 비활성화
            0,                       # RY 비활성화
            -quest_rot_delta[2]      # RZ만 활성화 (부호 반전!)
        ]) * SCALE_ROTATION

        # Rotation 데드존 + 안전 제한
        rot_magnitude = np.abs(fr5_rot[2])  # Z축만 체크
        if rot_magnitude < MIN_DELTA_ROT:
            fr5_rot = np.zeros(3)
        elif rot_magnitude > MAX_DELTA_ROT:
            print(f"   [회전 제한] {rot_magnitude:.2f}° → {MAX_DELTA_ROT}°")
            fr5_rot[2] = np.sign(fr5_rot[2]) * MAX_DELTA_ROT
    ...
    def calibrate(self, calibration_time=5.0):
        ...
        print(f"   Quest Z 회전:  {quest_rot_median[2]:.1f}°")
        print(f"   Robot TCP:     [{robot_tcp[0]:.1f}, {robot_tcp[1]:.1f}, {robot_tcp[2]:.1f}]")
        print(f"   Robot RZ:      {robot_tcp[5]:.1f}°")
    ...
    def run(self):
        """메인 루프"""
        print("\n" + "=" * 70)
        print("🎮 Quest-FR5 Teleoperation (Z-AXIS ROTATION ONLY)")
        print("=" * 70)
        print("\n📖 사용법:")
        ...
        print("   5. [q] - 종료")
        print("\n✅ Position: Full 3-DOF (X, Y, Z)")
        print("✅ Rotation: Z-axis only (엔드이펙터 끝단 회전)")
        print("   컨트롤러를 좌우로 비틀면 로봇 손목이 회전합니다.")
        print("=" * 70 + "\n")
            ...
            while self.running:
                ...
                if cmd == 'q':
                    ...
                    if not self.quest_connected:
                        ...
                        if self.calibrate():
                            print("🎬 텔레오퍼레이션 시작! (Position + Z-Rotation)")
                        else:
                            print("❌ 동기화 실패")
