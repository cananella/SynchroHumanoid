#!/usr/bin/env python3
"""
Quest-FR5 Teleoperation - X-AXIS ROTATION ONLY (Safe Test Version)

특징:
- Position: Full 3-DOF (X, Y, Z)
- Rotation: X-axis only (roll 회전)
- 안전한 스케일로 테스트
"""
...
SCALE_ROTATION_X = 1.0    # X축 회전 스케일 (1:1)
...
class QuestFR5TeleopAsync:
    ...
    def transform_quest_to_fr5(self, quest_pos_delta, quest_rot_delta):
        ...
        # Rotation 변환 (X축만 활성화!)
        fr5_rot = np.array([
            -quest_rot_delta[0] * SCALE_ROTATION_X,  # RX만 활성화 (부호 반전!)
            0,                                       # RY 비활성화
            0                                        # RZ 비활성화
        ])

        # Rotation 데드존 + 안전 제한
        rot_magnitude = np.abs(fr5_rot[0])  # X축만 체크
        if rot_magnitude < MIN_DELTA_ROT:
            fr5_rot = np.zeros(3)
        elif rot_magnitude > MAX_DELTA_ROT:
            print(f"   [회전 제한] {rot_magnitude:.2f}° → {MAX_DELTA_ROT}°")
            fr5_rot[0] = np.sign(fr5_rot[0]) * MAX_DELTA_ROT
    ...
    def calibrate(self, calibration_time=5.0):
        ...
        print(f"   Quest X 회전:  {quest_rot_median[0]:.1f}°")
        print(f"   Robot TCP:     [{robot_tcp[0]:.1f}, {robot_tcp[1]:.1f}, {robot_tcp[2]:.1f}]")
        print(f"   Robot RX:      {robot_tcp[3]:.1f}°")
    ...
    def run(self):
        """메인 루프"""
        print("\n" + "=" * 70)
        print("🎮 Quest-FR5 Teleoperation (X-AXIS ROTATION ONLY)")
        print("=" * 70)
        print("\n📖 사용법:")
        ...
        print("   5. [q] - 종료")
        print("\n✅ Position: Full 3-DOF (X, Y, Z)")
        print("⚠️  Rotation: X-axis only (roll 회전)")
        print("   컨트롤러를 좌우로 기울이면 로봇이 roll 회전합니다.")
        print(f"   스케일: {SCALE_ROTATION_X}, 최대: {MAX_DELTA_ROT}°")
        print("=" * 70 + "\n")
            ...
            while self.running:
                ...
                if cmd == 'q':
                    ...
                    if not self.quest_connected:
                        ...
                        if self.calibrate():
                            print("🎬 텔레오퍼레이션 시작! (Position + X-Rotation)")
                        else:
                            print("❌ 동기화 실패")
